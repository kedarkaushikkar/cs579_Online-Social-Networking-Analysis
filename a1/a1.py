...
def bfs(graph, root, max_depth):
    ...
    result_dict["visit"] = {}
    result_dict["path"] = {}
    result_dict["parent_list"] = {}
    queue = deque()
    for i in graph.nodes():
        if (i != root):
            result_dict["visit"][i] = 0
            result_dict["path"][i] = float("inf")
            result_dict["parent_list"][i] = []
        else:
            result_dict["visit"][i] = 0
            result_dict["path"][i] = 0
            result_dict["parent_list"][i] = [root]

    queue.append(root)
    while queue:
        node_consider = queue.popleft();
        if (result_dict["path"][node_consider] > max_depth):
            break
        for i in graph.neighbors(node_consider):
            if (result_dict["visit"][i] == 0):
                if (result_dict["path"][i] >= result_dict["path"][node_consider] + 1):
                    result_dict["path"][i] = result_dict["path"][node_consider] + 1
                    result_dict["parent_list"][i].append(node_consider)
                    queue.append(i)
        result_dict["visit"][node_consider] = 1

    # print result_dict

    for i in result_dict["visit"].keys():
        if (i != root):
            if (result_dict["visit"][i] == 1):
                node2distances[i] = result_dict["path"][i]
                node2num_paths[i] = len(result_dict["parent_list"][i])
                node2parents[i] = result_dict["parent_list"][i]
        else:
            node2distances[i] = result_dict["path"][i]
            node2num_paths[i] = len(result_dict["parent_list"][i])
...
def complexity_of_bfs(V, E, K):
    ...
    True
    """
    return float(V)*float(E)+float(math.log(K))
    pass
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    node2parents[root] = [root]
    leaf_distance = max(node2distances.values())
    leaf_list = []
    w = {}
    for key in node2distances.keys():
        if (node2distances[key] == leaf_distance):
    ...
    for i in sorted(node2distances, key=lambda x: node2distances[x], reverse=True):
        num_parents_weight = w[i] / len(node2parents[i])
        for node_consider in node2parents[i]:
            w[node_consider] = w[node_consider] + num_parents_weight
            tup = (node_consider, i)
            tup = tuple(sorted(tup))
            sort_edge.append(tup)

    for s in sort_edge:
        left_node, right_node = s[0], s[1]
        if (left_node in node2parents[right_node]):
            weight_edge = w[right_node] / len(node2parents[right_node])
        else:
            weight_edge = w[left_node] / len(node2parents[left_node])
        credit_dict[s] = weight_edge
    del credit_dict[(root, root)]
...
def approximate_betweenness(graph, max_depth):
    ...
    for i in graph.nodes():
        ...
        intermediate_credit_dict = bottom_up(i, node2distances, node2num_paths, node2parents)
        for key, value in intermediate_credit_dict.items():
            if (key in credit_dict.keys()):
                credit_dict[key] = credit_dict[key] + value
            else:
                credit_dict[key] = value

    for key, value in credit_dict.items():
        credit_dict[key] = float(value / 2)
    return credit_dict
    pass
...
def partition_girvan_newman(graph, max_depth):
    ...
    eb = sorted(approximate_betweenness(G, max_depth).items())
    edge_to_remove = sorted(eb , key=lambda x: x[:][1], reverse=True)
    i=0;
    while len(components) == 1:
        G.remove_edge(*edge_to_remove[i][0])
        components = [c for c in nx.connected_component_subgraphs(G)]
        i=i+1;
    return components
    pass
...
def get_subgraph(graph, min_degree):
    ...
    for name, degree in degrees.items():
        if degree >= min_degree:
            draw_nodes.append(name)

    graph_to_draw = graph.subgraph(draw_nodes)
...
def cut(S, T, graph):
    ...
    #            edges_in_cut_set += 1;
    edges_in_cut_set = 0;
    for s_node,t_node in [(s_list,t_list) for s_list in S for t_list in T]:
        if(graph.has_edge(s_node, t_node)):
            edges_in_cut_set += 1;
    return (edges_in_cut_set)
    pass
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
        ...
        result_score.append((i, norm_cut(components[0], components[1], graph)))
    return result_score
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    sorted_edges = sorted(graph.edges(test_node))
    edges_to_remove = sorted_edges[:n]
...
def jaccard(graph, node, k):
    ...
    for i in node_list:
        #print i
        check_edge=graph.has_edge(node,i)
        #print check_edge
        if(i!= node and not check_edge):
            i_neighbor = graph.neighbors(i)
            #print i_neighbor
            #print node_neighbor
            #print i_neighbor
            intersecting_nodes = [val for val in i_neighbor if val in node_neighbor]
            #print intersecting_nodes
            union_nodes = list(set().union(node_neighbor, i_neighbor))
            #union_nodes = [set(node_neighbor) | set(i_neighbor)]
            #print type(union_nodes)
            #print union_nodes
            edge = ((node,i), float(float(len(intersecting_nodes))/float(len(union_nodes))))
            jaccard_score.append(edge)
    #return jaccard_score
    sorted_by_second = sorted(jaccard_score, key=lambda tup: tup[0][1])
    sorted_by_second_1 = sorted(sorted_by_second, key=lambda tup: tup[1], reverse=True)
    return(sorted_by_second_1[:k])
    pass
...
def path_score(graph, root, k, beta):
    ...
    for i in node_list:
        #print i
        check_edge=graph.has_edge(root,i)
        #print check_edge
        if(i!= root and not check_edge):
            similarity_of_node = float(float(beta**node2distances[i]) * float(node2num_paths[i]))
            edge = ((root,i), similarity_of_node)
            path_score.append(edge)
    sorted_by_second = sorted(path_score, key=lambda tup: tup[0][1])
    sorted_by_second_1 = sorted(sorted_by_second, key=lambda tup: tup[1], reverse=True)
    return(sorted_by_second_1[:k])
    pass
...
def evaluate(predicted_edges, graph):
    ...
    0.5
    """
    denominator = len(predicted_edges)
    numerator = 0
    for i in predicted_edges:
        if(graph.has_edge(i[0], i[1])):
            numerator += 1
    evaluated_value = float(numerator)/ float(denominator)
